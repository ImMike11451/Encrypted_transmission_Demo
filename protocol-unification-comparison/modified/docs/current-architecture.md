# Encrypted Transmission Demo - Current Architecture Baseline
...
## 3. Current Client Identities

当前联调配置包含两个客户端：

| Client | Client ID | Config File | Purpose |
|---|---|---|---|
...
| 密钥校验 | Completed |
| 密钥注销 | Completed |
| AES-GCM 文本消息发送 | Completed |
| 服务端解密与接收方重加密存储 | Completed |
| 单条消息元数据查询 | Completed |
| 最近消息列表查询 | Completed |
| 双客户端联调配置 | Completed |
| 接收方获取密文并本地解密 | Not Implemented |
| 消息已读回执 | Not Implemented |
| 端到端加密 | Not Implemented |
| 长连接实时推送 | Not Implemented |

## 11. Known Structural Problems
...
## 12. Known Security Problems

当前识别出的安全问题包括：

1. 服务端可解密所有消息明文。
2. 客户端会在日志中输出协商后的会话密钥。
3. 服务端会在日志中输出解密后的消息明文。
4. SQL 使用字符串拼接方式构造，存在注入风险。
5. 完整 SQL 日志可能包含密钥或密文。
6. 数据库账号密码保存在版本控制中的配置文件里。
7. 测试私钥文件已经进入版本控制，需要明确其只用于演示环境。
8. 消息查询缺少严格的发送方/接收方权限控制。
9. 消息 ID 和审计 ID 基于秒级时间戳生成，存在并发碰撞可能。
10. Protobuf 解析结果检查和异常输入处理仍不完整。

## 13. Refactoring Constraints
